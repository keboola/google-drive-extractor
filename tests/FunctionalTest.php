...
YAML;
    ...
    public function testAutoGeneratedHeader(): void
    {
        $filePath = __DIR__ . '/data/in/auto_header.csv';
        touch($filePath);
        $csv = new CsvWriter($filePath);
        $this->writeLineToCsvFile($csv, 1, 7);
        $this->writeLineToCsvFile($csv, 2, 5);
        ...
        $this->testFileName = 'auto_header';
        $this->testFile = $this->prepareTestFile($filePath, 'auto_header');
        $this->config = $this->makeConfig($this->testFile);

        // Auto-generate Excel-style headers, treat all rows as data
        $this->config['parameters']['sheets'][0]['header']['rows'] = -1;
        ...
        $this->assertFileEquals(
            __DIR__ . '/data/expectedFiles/auto_header.csv',
            $this->dataPath . '/out/tables/' . $this->getOutputFileName($fileId, $sheetId),
        );
    }
